...
features_path = r"C:\Users\Salin\OneDrive\Documentos\ESSEX\DSPROJECT\Participant_Analysis_Int_more_features_with_Baseline\All_Participants_PoincareMetrics.csv"
...
output_path = r"C:\Users\Salin\OneDrive\Documentos\ESSEX\DSPROJECT\Participant_Analysis_Int_more_features_with_Baseline\All_Participants_PoincareMetrics_normalized.csv"
...
for index, row in video_df.iterrows():
    participant = row["Participant"]
    video_name = row["Stimulus"]
    baseline_row = find_baseline(participant, video_name)
    
    if baseline_row is not None:
        normalized_features = {"Participant": participant, "Stimulus": video_name}
        
        # Normalizing each feature (skip non-numeric columns)
        for col in df.columns[2:]:  # Skipping 'Participant' and 'Stimulus'
