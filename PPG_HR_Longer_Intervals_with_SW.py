...
# Sliding windows to apply to the already cleaned data
def add_sliding_windows(df, window_size=30, step_size=10, min_window_length=10, tail_min_length=5):
    all_windows = []
    
    for stim_name, group in df.groupby("SourceStimuliName"):
        duration = group["Timestamp"].max()
        window_count = 0  # Tracking how many valid windows were created for this stimulus

        if duration < tail_min_length:
            print(f"Stimulus '{stim_name}' skipped (duration < {tail_min_length}s)")
            continue

        # If the entire stimulus is between 10s and 30s, full window
        if duration < window_size and duration >= min_window_length:
            window_df = group.copy()
            window_df["Window_Start"] = 0
            window_df["Window_End"] = duration
            window_df["Window_Index"] = "w1"
            all_windows.append(window_df)
            window_count += 1
            continue

        # Sliding windows for normal duration
        start = 0
        window_index = 1
        while start + min_window_length <= duration:
            end = min(start + window_size, duration)
            window_df = group[(group["Timestamp"] >= start) & (group["Timestamp"] < end)].copy()

            if len(window_df) < 2:
                print(f"Not enough data for stimulus '{stim_name}', window '{window_index}'")
                start += step_size
                continue

            window_df["Window_Start"] = start
            window_df["Window_End"] = end
            window_df["Window_Index"] = f"w{window_index}"
            all_windows.append(window_df)
            window_count += 1
        ...
        # Adding tail if leftover is between 5s and 10s
        if start < duration and (duration - start) >= tail_min_length:
            end = duration
            window_df = group[(group["Timestamp"] >= start) & (group["Timestamp"] < end)].copy()
            if len(window_df) > 1:
                window_df["Window_Start"] = start
                window_df["Window_End"] = end
                window_df["Window_Index"] = f"w{window_index}_tail"
                all_windows.append(window_df)
                window_count += 1
            else:
                print(f"Not enough data for stimulus '{stim_name}', tail window '{window_index}'")

        # If no windows were created for a valid stimulus
        if window_count == 0:
            print(f"No valid windows created for stimulus '{stim_name}' (duration = {duration:.2f}s)")

    return pd.concat(all_windows, axis=0) if all_windows else pd.DataFrame()
...
output_folder = r"c:\Users\Salin\OneDrive\Documentos\ESSEX\DSPROJECT\PPG_HR_Analysis_Longer_Intervals_with_SW\PPG_HR_Analysis_Longer_Intervals"
output_folder_SW = r"c:\Users\Salin\OneDrive\Documentos\ESSEX\DSPROJECT\PPG_HR_Analysis_Longer_Intervals_with_SW\PPG_HR_Analysis_Longer_Intervals_SW"

## Ensuring the output folders are NEW each time
...
if os.path.exists(output_folder_SW):
    shutil.rmtree(output_folder_SW)  # Deleting existing folder and its contents
os.makedirs(output_folder_SW) # Creating a fresh folder 
...
for filename in os.listdir(input_folder):
    if filename.endswith(".csv"):
      ...
      # Adding Baseline stimuli (no splitting required)
        ...
        windowed_data = add_sliding_windows(final_data)

        final_output_path = os.path.join(output_folder, f"filtered_{filename}")
        final_data.to_csv(final_output_path, index=False)
        
      
        final_output_path_sw = os.path.join(output_folder_SW, f"filtered_{filename}")
        windowed_data.to_csv(final_output_path_sw,index=False)
